...
def cluster_stocks_view(request, stocks: str):
    ...
    def elbow_curve_plot():
        distortion, _, _, _, _ = make_kmeans_cluster_dataframe(timeframe, chosen_k, asx_codes)
    ...
    def cluster_plot():
        _, _, centroids, idx, data_df = make_kmeans_cluster_dataframe(timeframe, chosen_k, asx_codes)
        centroids_df = pd.DataFrame.from_records(centroids, columns=["return", "volatility"])
        return (p9.ggplot(data_df, p9.aes("return", "volatility", colour="factor(cluster_id)"))
                + p9.geom_point(show_legend=False, size=3)
