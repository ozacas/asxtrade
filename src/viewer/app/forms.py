...
class WorldBankSCMForm(forms.Form): # multi-country, single metric
    countries = forms.MultipleChoiceField(choices=(), required=True, widget=forms.widgets.SelectMultiple(attrs={'size': 12}))
    topic = forms.ChoiceField(choices=(), required=True)
    indicator = DynamicChoiceField(choices=(), validators=[], required=True) # only indicators which have ALL selected countries will be shown from the given topic
